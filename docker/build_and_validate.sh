#!/usr/bin/env bash
# Phase 2 regression + Phase 4/5 build and validation, end to end.
#
...
echo "== [2-8] build + run + validate in container =="
docker run --rm -v "${REPO_ROOT}:/work" -w /work "${IMAGE}" bash -lc '
  source /opt/ros/jazzy/setup.bash
  set -eo pipefail
  cd /work/ros2_ws
  echo "== [2/8] colcon build =="
  ...
  echo "== [5/8] Phase 4 controller matrix =="
